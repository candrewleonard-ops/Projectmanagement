// Copies the Next.js standalone build output into desktop-mac/next-app
// so electron-builder can package it inside the .app bundle.
const fs = require("fs");
...
if (!fs.existsSync(srcStandalone)) {
  console.error("[copy-standalone] .next/standalone not found. Run: cd .. && DESKTOP_BUILD=true npm run build");
  process.exit(1);
}
